...
import { Home, AlertTriangle, CheckCircle, ArrowRight, Copy, CreditCard, User, Hash, Banknote, Bitcoin, Network, AlertOctagon, Wallet, ArrowLeft } from 'lucide-react';
...
const LocalPaymentPage: React.FC = () => {
    ...
    const generateInvoiceMessage = (details: { email: string; phone: string; }) => {
        return `
    ...
    const handleDetailsSubmit = async (details: { email: string; phone: string; }) => {
        // Check if customer is banned
        const banStatus = await trafficService.checkCustomerBan(details.email, details.phone);
        if (banStatus.banned) {
            throw new Error(banStatus.message || t.accessDenied || 'Access Denied');
        }
        ...
        console.log('Discord Webhook URL:', settings.discord_webhook_url ? 'Present' : 'Missing');

        if (!product || !contactUrl) {
            console.error('Missing product or contact URL');
            return;
        }
        
        // Send Discord Notification (Main Webhook - Payment Confirmed)
        // Uses the dedicated "Paid" webhook setting
        const paidWebhookUrl = settings.discord_webhook_url_paid;
        
        if (paidWebhookUrl) {
            console.log('Initiating Discord notification (Paid Webhook)...');
            // Determine avatar URL: prefer explicit setting, fallback to site logo
            let avatarUrl = settings.discord_bot_avatar_url;
            
            if (!avatarUrl && settings.site_logo_url) {
                // Handle relative vs absolute URLs and ensure encoding
                let logoUrl = settings.site_logo_url;
                if (!logoUrl.startsWith('http')) {
                    logoUrl = window.location.origin + (logoUrl.startsWith('/') ? '' : '/') + logoUrl;
                }
                // Encode the URL to handle spaces (e.g. "cheatloop copy.png")
                avatarUrl = encodeURI(logoUrl);
            }

            // Add cache buster if it's a supabase URL to ensure Discord sees the latest version
            if (avatarUrl && avatarUrl.includes('supabase.co')) {
                const separator = avatarUrl.includes('?') ? '&' : '?';
                avatarUrl = `${avatarUrl}${separator}t=${Date.now()}`;
            }
            
            console.log('Using Discord Avatar URL:', avatarUrl);

            discordService.sendPurchaseNotification(paidWebhookUrl, {
                productName: product.title,
                price: displayPrice,
                paymentMethod: `${method?.method_name} (${method?.country})`,
                customerEmail: details.email,
                customerPhone: details.phone,
                orderId: Math.random().toString(36).substring(7).toUpperCase(),
                title: '✅ Payment Confirmed (I Have Paid)'
            }, avatarUrl, settings.discord_paid_admin_id || settings.discord_admin_id)
            .then(() => console.log('Discord notification promise completed'))
            .catch(err => console.error('Discord notification promise failed:', err));
        } else {
            console.warn('Skipping Discord notification: No "Paid" webhook URL configured');
        }

        const message = generateInvoiceMessage(details);
        
        const whatsappBaseUrl = settings.whatsapp_url || 'https://api.whatsapp.com/send?phone=9647832941204';
        const separator = whatsappBaseUrl.includes('?') ? '&' : '?';
        const whatsappUrl = `${whatsappBaseUrl}${separator}text=${encodeURIComponent(message)}`;
            ...
            <div className="relative z-10 container mx-auto px-4 py-12 max-w-2xl">
                <div className="bg-[#0a0a0c]/80 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-8 relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                        ...
                        <Link 
                            to={`/pre-purchase/${productId}`}
                            className="w-full py-3 px-6 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white font-medium rounded-xl transition-all duration-300 flex items-center justify-center gap-2 border border-white/5"
                        >
                            <ArrowLeft className={`w-5 h-5 transition-transform ${lang === 'ar' ? 'rotate-180' : ''}`} />
